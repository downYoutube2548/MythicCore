...
public class Swirl extends TriggerAuraReaction {
    ...
    public boolean trigger(DamagePacket damage, double gauge_unit, String decay_rate, LivingEntity entity, @Nullable Entity damager, StatProvider stats, EntityDamageEvent.DamageCause damage_cause) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(MythicCore.getInstance(), ()->{
            ...
            double resistance_multiplier = StatCalculation.getResistanceMultiplier(entity, getConfig().getString("damage-element"));
            ...
            String finalSwirled_aura_gauge = swirled_aura_gauge;
            Bukkit.getScheduler().runTask(MythicCore.getInstance(), ()->{
                ...
                for (Entity swirl_entity : swirl_entities) {
                    ...
                    if (swirl_entity instanceof LivingEntity swirled_living_entity && !swirled_living_entity.isInvulnerable()) {
                        damage(final_damage, damager, swirled_living_entity, Objects.requireNonNull(getConfig().getConfigurationSection("aura-overriding")).getKeys(false).contains(this.aura) ? getConfig().getString("aura-overriding."+this.aura) : this.aura, false, Double.parseDouble(Utils.splitTextAndNumber(finalSwirled_aura_gauge)[0]), Utils.splitTextAndNumber(finalSwirled_aura_gauge)[1], "SWIRL_"+this.aura, 0, false, damage_cause);
                    }
                }
            });
