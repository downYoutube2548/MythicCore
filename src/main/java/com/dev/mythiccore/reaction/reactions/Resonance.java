...
public class Resonance extends SingleReaction {
    ...
    public boolean trigger(DamagePacket damage, double gauge_unit, String decay_rate, LivingEntity entity, @Nullable Entity damager, StatProvider stats, EntityDamageEvent.DamageCause damage_cause) {
        ...
        for (LivingEntity living_entity : entities) {
            ...
            Bukkit.getScheduler().runTaskLater(MythicCore.getInstance(), ()-> damage(final_damage, damager, living_entity, this.getTrigger(), false, true, true, (getAuraData(living_entity.getUniqueId()).getMapAura().containsKey(getTrigger())) ? 1 : 0, ConfigLoader.getDefaultDecayRate(), "RESONANCE_REACTION", 0, false, damage_cause), (long) (getConfig().getDouble("delay-tick-per-block") * distance));
